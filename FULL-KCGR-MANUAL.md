...
| cron-job.org API Key (`kcgr-admin-cronjob-read`) | Generated from the cron-job.org account's own Settings page | Whoever can reach `~/.kcgr_secrets/credentials.env` on graywolf, or log into the cron-job.org account directly | Read-only — `GET /jobs`, job status for both trigger jobs. No IP restriction: graywolf's WAN IP is confirmed DHCP-assigned (router settings, 9/14/2026), so restricting would have silently broken the key at the next lease rotation | 🔧 |
| Cloudflare API Token (`kcgr-admin-cloudflare-read`) | Generated from Cloudflare's My Profile → API Tokens (user-scoped, `cfut_` prefix) | Whoever can reach `~/.kcgr_secrets/credentials.env` on graywolf, or the Cloudflare account directly | `Workers Scripts: Read`, scoped to this account only — 1-year TTL (not "never expire": this only feeds a status display, so an expiration is visible and low-stakes rather than silent, unlike the never-expire tokens above). No IP restriction, same reasoning as the cron-job.org key | 🔧 |
