...
DEBUG LOGGING (added for troubleshooting, 8/14/2026):
Every raw packet seen during the listen window is now printed with a
[RAW] prefix, before any filtering happens. This is temporary
diagnostic logging - remove once the current "beacon reaches Graywolf/
aprs.fi but never appears in the poller's own output" issue is
resolved, since it will make the Actions log noisy on a normal run.
...
def listen_for_kcgr_objects(listen_seconds: int, callsign: str) -> list:
    ...
    def _on_packet(raw_packet):
        # --- DEBUG (added 8/14/2026): log every packet seen, before any
        # filtering, so we can tell whether a given beacon reached this
        # poller at all versus being filtered/dropped downstream. Remove
        # once the current investigation is resolved - noisy on a normal
        # run otherwise.
